...
def matrix_mul(m_a, m_b):
    ...
    if not all((isinstance(em, int) or isinstance(em, float))
            for em in [num for row in m_a for num in row]):
        raise TypeError("m_a should contain only integers or floats")
    if not all((isinstance(em, int) or isinstance(em, float))
            for em in [num for row in m_b for num in row]):
